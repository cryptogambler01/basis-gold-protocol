...
contract StableFund is Ownable, Operator {
    ...
    constructor(
        ...
        pair = IUniswapV2Pair(
            UniswapV2Library.pairFor(_factory, _bsg, _dai)
        );
        path = [_bsg, _dai];
        dai = IERC20(_dai);
        bsg = IERC20(_bsg);
    ...
    function getXAUTPrice() internal view returns (uint256) {
        try goldOracle.goldPriceOne() returns (uint256 price) {
            return price;
        } catch {
            revert('StableFund: failed to consult gold price from the oracle');
        }
    ...
    // set to 90% of XAU Oracle Price
    function goldPriceCeiling() internal view returns(uint256) {
        uint256 tReturn = getXAUTPrice().mul(uint256(goldBuyPercentage)).div(100);
        //console.log("Price Ceiling: ", tReturn);
        return tReturn;
    }

    function goldPriceFloor() internal view returns(uint256) {
        uint256 tReturn = getXAUTPrice().mul(uint256(goldSellPercentage)).div(100);
        //console.log("Price Floor: ", tReturn);
        return tReturn;
    }

    //Selling Functions
    //THE MATH - https://info.etherscan.com/ethereum-unit-converter/
    function sellBSGtoStableFund(uint256 numTokens) public checkMigration
    {
        ...
        require(
            numTokens.mul(goldPriceCeiling()).div(1e18) < IERC20(dai).balanceOf(address(this)),
        ...
        dai.transfer(msg.sender, numTokens.mul(goldPriceCeiling()).div(1e18));
    ...
    function sellBSGOverPeg(uint256 numTokens) public  checkMigration 
    {
        ...
        require(numTokens <= uint256(5).mul(1e18), 'Stable Fund: cannot sell more than 5 BSG at a time');
        ...
        console.log("Made it past the allowance: ", allowance);
        console.log("How many we selling: ", numTokens);
        //only component that isnt testing in the testing harness.
        console.log(IERC20(dai).balanceOf(address(this)));
        console.log(IERC20(bsg).balanceOf(address(this)));
